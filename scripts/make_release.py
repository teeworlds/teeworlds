...
valid_platforms = ["win32", "win64", "macos",
                   "linux_x86", "linux_x86_64", "src"]
...
if use_bundle:
    ...
    clientbundle_content_dir = os.path.join(
        package_dir, "Teeworlds.app/Contents")
    clientbundle_bin_dir = os.path.join(clientbundle_content_dir, "MacOS")
    clientbundle_resource_dir = os.path.join(
        clientbundle_content_dir, "Resources")
    clientbundle_framework_dir = os.path.join(
        clientbundle_content_dir, "Frameworks")
    ...
    shell("cp /usr/local/opt/freetype/lib/libfreetype.6.dylib " +
          clientbundle_framework_dir)
    shell("cp /usr/local/opt/libpng/lib/libpng16.16.dylib " +
          clientbundle_framework_dir)
    shell("cp /usr/local/opt/sdl2/lib/libSDL2-2.0.0.dylib " +
          clientbundle_framework_dir)
...
<dict>
    ...
    serverbundle_content_dir = os.path.join(
        package_dir, "Teeworlds Server.app/Contents")
    serverbundle_bin_dir = os.path.join(serverbundle_content_dir, "MacOS")
    serverbundle_resource_dir = os.path.join(
        serverbundle_content_dir, "Resources")
    ...
    shutil.copy(source_package_dir+"serverlaunch"+exe_ext,
                serverbundle_bin_dir + "/"+name+"_server")
...
if use_dmg:
    ...
    shell("hdiutil create -srcfolder %s -volname Teeworlds -quiet %s_temp" %
          (package_dir, package))
    shell("hdiutil convert %s_temp.dmg -format UDBZ -o %s.dmg -quiet" %
          (package, package))
