...
def get_server_info(address):
    ...
    try:
        ...
        while not token_success and retries > 0:
            retries -= 1
            token_success, token_cl, token_srv = send_token(
                sock, address, sock.gettimeout() * 2)
        ...
        def send_header(sock, address, token_cl, token_srv, timeout=TIMEOUT, sleep_secs=None):
            # Get info request
            sock.sendto(header_connless(token_srv, token_cl) +
                        PACKET_GETINFO + b'\x00', address)
            sock.settimeout(timeout)
        ...
        while not data_success and retries > 0:
            retries -= 1
            data_success, data = send_header(
                sock, address, token_cl, token_srv, sock.gettimeout() * 2)
        ...
        if FORCE_SLEEP and not data_success and retries == 0:
            ...
            while not data_success and retries > 0:
                ...
                sleep_secs *= 2
                data_success, data = send_header(
                    sock, address, token_cl, token_srv, sock.gettimeout() * 2.0, sleep_secs=sleep_secs)
            if not data_success and retries == 0:
                raise ValueError(
                    f"Failed to retrieve server info from {address}")
...
def get_list(address):
    ...
    try:
        ...
        token_cl, token_srv = unpack_control_msg_with_token(data)
        assert token_cl == token, "Master %s send wrong token: %d (%d expected)" % (
            address, token_cl, token)
        answer = True

        # Get list request
        sock.sendto(header_connless(
            token_srv, token_cl) + PACKET_GETLIST, addr)
        head = header_connless(token_cl, token_srv) + PACKET_LIST

        while 1:
            data, addr = sock.recvfrom(BUFFER_SIZE)
            # Header should keep consistent
            assert data[:len(head)] == head, "Master %s list header mismatch: %r != %r (expected)" % (
                address, data[:len(head)], head)
...
if __name__ == '__main__':
    ...
    for i in range(1, NUM_MASTERSERVERS+1):
        m = Master_Server_Info(
            ("master%d.teeworlds.com" % i, MASTERSERVER_PORT))
        ...
        if servers_info[0].finished == True:
            if servers_info[0].info:
                ...
                if server_info["num_players"] > server_info["max_players"] \
                    ...
                    print('> Server %s has %s' %
                          (server_info["address"], server_info["bad"]))
                # check actual purity
    ...
    print('%d players (%d bots) and %d spectators (%d bots)' % (
        num_players, num_botplayers, num_clients - num_players, num_botspectators))
