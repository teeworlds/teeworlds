...
if len(sys.argv) != 2:
	...
	print(sys.argv[0], "mod language directory")
	sys.exit(-1)
...
def generate_languagefile(outputfilename, srctable, loctable, defaultloctable):
	...
	for k in srctable_keys:
		if k in loctable and len(loctable[k]) and not k in defaultloctable:
			content += k + "\n== ".encode() + loctable[k] + "\n\n".encode()
			num_items += 1

	content += "##### needs translation #####\n\n".encode()
	for k in srctable_keys:
		if (not k in loctable or len(loctable[k]) == 0) and not k in defaultloctable:
			content += k + "\n== \n\n".encode()
			num_items += 1
			new_items += 1
...
for filename in os.listdir(mod_dir):
	...
	if mod_dir[-1] != '/':
		mod_dir += '/'
	defaultfilename = "data/languages/" + filename
	filename = mod_dir + filename
	generate_languagefile(filename, srctable, load_languagefile(filename), load_languagefile(defaultfilename))
