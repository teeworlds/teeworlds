...
class Dmg:
    ...
    def _check_call(self, process_args, *args, **kwargs):
        if self.config.verbose >= 1:
            print("EXECUTING {}".format(" ".join(shlex.quote(x)
                  for x in process_args)))
...
class Dmgtools(Dmg):
    ...
    def create(self, dmg, volume_name, directory, symlinks):
        input_size = sum(os.stat(os.path.join(path, f)).st_size for path,
                         dirs, files in os.walk(directory) for f in files)
...
class Hdiutil(Dmg):
    ...
    def create(self, dmg, volume_name, directory, symlinks):
        ...
        for i in range(5):
            ...
            try:
                self._hdiutil('create', '-volname', volume_name,
                              '-srcdir', directory, dmg)
...
def main():
    ...
    subcommands = p.add_subparsers(
        help="Subcommand", dest='command', metavar="COMMAND")
    subcommands.required = True

    create = subcommands.add_parser(
        "create", help="Create a dmg archive from files or directories")
    create.add_argument('-v', '--verbose', action='count',
                        help="Verbose output")
    ...
    createx.add_argument(
        '--hdiutil', help="Path to the hdiutil (only exists for macOS at time of writing)")
    create.add_argument('output', metavar="OUTPUT",
                        help="Filename of the output dmg archive")
    create.add_argument('volume_name', metavar="VOLUME_NAME",
                        help="Name of the dmg archive")
    create.add_argument('directory', metavar="DIR",
                        help="Directory to create the archive from")
    create.add_argument('--symlink', metavar="SYMLINK", nargs=2, action="append",
                        help="Symlink the first argument under the second name")
    args = p.parse_args()
    ...
    if args.dmgtools:
        ...
        dmg = Dmgtools(ConfigDmgtools(dmg=dmg, hfsplus=hfsplus,
                       newfs_hfs=newfs_hfs, verbose=verbose))
    elif args.hdiutil:
        dmg = Hdiutil(ConfigHdiutil(hdiutil=args.hdiutil, verbose=verbose))
    else:
        raise RuntimeError("unreachable")
    dmg.create(volume_name=args.volume_name, directory=args.directory,
               dmg=args.output, symlinks=symlinks)
