...
void CMenus::RenderMenubar(CUIRect Rect)
{
	...
	if(Client()->State() == IClient::STATE_ONLINE)
	{
		...
		Box.VSplitRight(ButtonWidth*2.0f + Spacing, 0, &Right);
		...
		Right.VSplitRight(Spacing, &Right, 0);
		Right.VSplitRight(ButtonWidth/2.0f, &Right, &Button);
		static CButtonContainer s_SettingsButton;
		if(DoButton_MenuTabTop(&s_SettingsButton, "\xE2\x9A\x99", m_ActivePage == PAGE_SETTINGS, &Button, Alpha, Alpha) || CheckHotKey(KEY_S))
			NewPage = PAGE_SETTINGS;

		Right.VSplitRight(ButtonWidth/2.0f, &Right, &Button);
		static CButtonContainer s_ServerBrowserButton;
		if(DoButton_SpriteID(&s_ServerBrowserButton, IMAGE_BROWSER, SPRITE_BROWSER, m_GamePage == PAGE_INTERNET || m_GamePage == PAGE_LAN, &Button) || CheckHotKey(KEY_B))
