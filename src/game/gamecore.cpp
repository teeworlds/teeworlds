...
void CCharacterCore::Tick(bool UseInput)
{
	...
	{
		vec2 NewPos;
		NewPos = m_pCollision->TestBoxWater(m_HookPos, vec2(1.0f, 1.0f)) ? m_HookPos + m_HookDir * m_pWorld->m_Tuning.m_HookFireSpeed * m_pWorld->m_Tuning.m_HookLiquidSlowdown : m_HookPos + m_HookDir * m_pWorld->m_Tuning.m_HookFireSpeed;
...
{
	return (m_pCollision->TestBoxWater(vec2(m_Pos.x, m_Pos.y), vec2(PHYS_SIZE, PHYS_SIZE + 1.0f) * (2.0f / 3.0f)));
}

bool CCharacterCore::IsFloating()
{ 
	return (!m_pCollision->TestBoxWater(vec2(m_Pos.x, m_Pos.y - 2.0f), vec2(PHYS_SIZE, PHYS_SIZE + 1.0f) * (2.0f / 3.0f)));
}
