	...
	if(m_VoteCloseTime)
	{
		...
		else
		{
			...
			if(m_VoteUpdate)
			{
				...
				for(int i = 0; i < MAX_CLIENTS; i++)
				{
					if(!m_apPlayers[i] && SvSpecCantVote || m_apPlayers[i]->GetTeam() == -1 && SvSpecCantVote || aVoteChecked[i] && SvSpecCantVote)	// don't count in votes by spectators
						continue;
					if(m_VoteKick && 
