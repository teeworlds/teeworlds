...
void CGameContext::OnClientAuthCallback(IConsole::CCommandInfo *pCmd, void *pUser)
{
	CGameContext *pContext = (CGameContext *)pUser;
	CNetMsg_Sv_RegisterRconCommand Msg;
	Msg.m_pName = pCmd->m_pName;
	Msg.m_pParams = pCmd->m_pParams;
	Msg.m_pHelp = pCmd->m_pHelp;
	pContext->Server()->SendPackMsg(&Msg, MSGFLAG_VITAL, pContext->m_pCurentClientID);
}

void CGameContext::OnClientAuth(int ClientID)
{
	m_pCurentClientID = ClientID;
	Console()->GetCommandsInfo(CFGFLAG_SERVER, OnClientAuthCallback, this);
}
