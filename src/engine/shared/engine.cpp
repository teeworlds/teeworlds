...
void CEngine::Con_DbgDumpmem(IConsole::IResult *pResult, void *pUserData)
{
...
void CEngine::Con_DbgLognetwork(IConsole::IResult *pResult, void *pUserData)
{
	CEngine *pEngine = static_cast<CEngine *>(pUserData);
...
CEngine::CEngine(const char *pAppname)
{
	dbg_logger_stdout();
	dbg_logger_debugger();
...
void CEngine::HostLookup(CHostLookup *pLookup, const char *pHostname, int Nettype)
{
...
void CEngine::AddJob(CJob *pJob, JOBFUNC pfnFunc, void *pData)
{
	if(g_Config.m_Debug)
